...
function InterfaceTheme() {
  ...
  const saved = getThemePrefs();
  const [accent, setAccent]           = useState<AccentColor>(saved.accent);
  const [fontSize, setFontSize]       = useState<FontSize>(saved.fontSize);
  const [animations, setAnimations]   = useState<AnimationLevel>(saved.animations);
  const [glassEffect, setGlassEffect] = useState(saved.glassEffect);

  const preview = (patch: Partial<{ accent: AccentColor; fontSize: FontSize; animations: AnimationLevel; glassEffect: boolean }>) => {
    applyTheme({ accent, fontSize, animations, glassEffect, ...patch });
  };

  const handleSave = () => {
    const prefs = { accent, fontSize, animations, glassEffect };
    ...
    <main className="px-5 pt-10 pb-28">
      ...
      <div className="mb-4 rounded-2xl glass p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {ACCENT_COLORS.map((c) => (
            <button
              key={c.id}
              onClick={() => { setAccent(c.id); preview({ accent: c.id }); }}
      ...
      <div className="mb-4 rounded-2xl glass p-4 space-y-3">
        ...
        <div className="flex gap-2">
          {FONT_SIZES.map((f) => (
            <button
              key={f.id}
              onClick={() => { setFontSize(f.id); preview({ fontSize: f.id }); }}
      ...
      <div className="mb-4 rounded-2xl glass p-4 space-y-3">
        ...
        <div className="flex gap-2">
          {(["full", "reduced", "none"] as AnimationLevel[]).map((a) => (
            <button
              key={a}
              onClick={() => { setAnimations(a); preview({ animations: a }); }}
      ...
      <div className="mb-6 rounded-2xl glass p-4">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => { setGlassEffect((v) => { const next = !v; preview({ glassEffect: next }); return next; }); }}
      ...
      <p className="mt-3 text-center text-[11px] text-muted-foreground">
        Changes preview instantly — tap Save to keep them permanently
      </p>
    </main>
  );
}
